...
# Add the RoboTwin-Mem root so envs can be imported.
robowin_root = Path(__file__).resolve().parents[4]
# Ensure RoboTwin-Mem root is the first search in order to use envs.
if str(robowin_root) not in sys.path:
    sys.path.insert(0, str(robowin_root))
# Ensure CWD is RoboTwin-Mem in order to load assets.
import os
...
def load_task_instructions():
    instruction_root = robowin_root / "description" / "task_instruction"
    task_instructions = {}
    for task_name in ALL_TASKS:
        instruction_path = instruction_root / f"{task_name}.json"
        with instruction_path.open("r", encoding="utf-8") as f:
            task_data = json.load(f)
        seen_instructions = task_data.get("seen") or []
        instruction = (task_data.get("full_description") or (seen_instructions[0] if seen_instructions else "")).strip()
        if not instruction:
            raise ValueError(f"No instruction found for task '{task_name}' in {instruction_path}")
