...
class EventVLA(baseframework):
    ...
    _EVENTVLA_MODE = "pure_image_keyframe_memory"
    ...
    def __init__(self, config: Optional[dict] = None, **kwargs) -> None:
        ...
        if not self.memory_injection_enabled:
            raise ValueError(
                "EventVLA requires keyframe image injection to be enabled."
            )
        ...
        if memory_enable:
            raise ValueError("EventVLA uses raw keyframe image memory and requires framework.memory_buffer.enable=false.")
        ...
        if provide_teacher_commit_images:
            raise ValueError(
                "EventVLA uses raw keyframe image memory; set "
        ...
        logger.info(
            "EventVLA pure_image_keyframe_memory enabled: temporal anchors absolute_indices=[0], "
            "delta_indices=[-30, -15, 0], max_keyframe_images=%s.",
    ...
    def _use_keyframe_image_inputs(self) -> bool:
        return (
            ...
            and self.memory_injection_mode == self._EVENTVLA_MODE
        )
    ...
    def predict_action(self, examples: List[dict] = None, **kwargs: str) -> np.ndarray:
        ...
        if memory_update_only:
            return {
                ...
                "message": "raw keyframe image mode stores memory in the eval interface",
            }
